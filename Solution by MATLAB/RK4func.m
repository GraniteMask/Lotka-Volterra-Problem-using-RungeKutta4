function [x,t] = RK4func(f, x0, t0, tf, dt) % f=function describing differential equation system, x0=initial condition, t0=initial time, tf=final time, dt=sample time
    %% RUNGE KUTTA FORMULA
    ...
    for k = 1:nt-1
        ...
        dx=(k1 + 2*k2 + 2*k3 +k4)/6;      % since k2, k3 represent a more precise expression of current and next state, these two entitites are twice as k1 and k4
        x(:,k+1) = x(:,k) + dx;   
